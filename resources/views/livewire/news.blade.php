    ...
    @else
        <div class="space-y-4">
            @foreach($articles as $article)
                <div wire:click="openArticle('{{ $article['link'] }}')" class="block rounded-lg border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-800 overflow-hidden transition ">
                    @if($article['image'])
